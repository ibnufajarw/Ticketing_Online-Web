...
class AcaraController extends Controller
{
    ...
    public function datatableJson(Request $request)
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('status', function ($data) {
                $status =  $data->status == 'draft' ? '<span class="badge badge-secondary">Draft</span>' : '<span class="badge badge-success">Published</span>';
            ...
            ->addColumn('aksi', function ($data) use ($request) {
                ...
                if (auth()->user()->role == 'admin') {
                    $statusButton = '<a href="#" data-toggle="modal" data-target="#confirm" data-id="' . $data->id . '" class="dropdown-item konfirmasi">Ubah Status</a>';
                }
                // }
                ...
                <div class="btn-group" role="group">
                    ...
                    <div class="dropdown-menu">
                        <a href="' . route('dashboard.acara.edit', $data->id) . '" class="dropdown-item">Edit</a>
                        <a href="' . route('dashboard.acara.edit', $data->id) . '" class="dropdown-item">Detail</a>
                        <a href="' . route('dashboard.jenis-tiket.index', [$data->id, 'jenis' => $jenis]) . '" class="dropdown-item">Kelola Tiket</a>
                        ' . $statusButton . '
                    </div>
                </div>
                ';
                ...
                <div class="btn-group" role="group">
                    ...
                    <div class="dropdown-menu">
                        <a href="' . route('dashboard.acara.edit', $data->id) . '" class="dropdown-item">Edit</a>
                        <a href="' . route('dashboard.acara.edit', $data->id) . '" class="dropdown-item">Detail</a>
                        <a href="' . route('dashboard.jenis-tiket.index', $data->id) . '" class="dropdown-item">Kelola Tiket</a>
                        ' . $statusButton . '
                    </div>
                </div>
                ';
    ...
    public function store(AcaraRequest $request)
    {
        ...
        try {
            ...
            return redirect()->route('dashboard.acara.index')->with('success', 'Acara berhasil dibuat tunggu konfirmasi 1x24 jam');
        } catch (Exception $e) {
            // dd($e->getMessage());
            DB::rollback();
    ...
    public function edit(Acara $acara)
    {
        ...
        $lokasi = DB::table('lokasi')->orderBy('nama')->get();
        $kampus = DB::table('kampus')->orderBy('nama')->get();
        if (auth()->user()->role != 'admin' && $acara->user_id != auth()->user()->id) {
            return abort(404);
        }
    ...
    public function landingPageShow($slug)
    {
        ...
        $jenisTiket = JenisTiket::find($acara->jenisTiket->id ?? 0);
        return view('landing-page.acara.show', compact('acara', 'tiketGratis', 'tiketBerbayar', 'jenisTiket'));
    }
    ...
    public function beliTiket(Request $request)
    {
        ...
        if (!$tiket->harga) {
            ...
            return redirect()->route('transaksi.detail', $transaksi->kode_transaksi);
        } else {
            $transaksi = Transaksi::create([
                'user_id' => auth()->user()->id,
                'tiket_id' => $tiket->id,
    ...
    public function confirm(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Berhasil di konfirmasi');
    }
}
