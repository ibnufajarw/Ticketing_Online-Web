            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('dashboard.metode-pembayaran.create') }}" class="btn btn-sm btn-primary">Tambah Data</a>
                    </div>
                </div>
                <div class="card-body">
    ...
    <script>
        $(function() {
            const table = $('table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('dashboard.metode-pembayaran.datatable-json') }}",
                    type: 'GET',
                    headers: {
            ...
            $(document).on('click', '.btn-danger', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ url('/dashboard/metode-pembayaran') }}/"+id,
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': "{{ csrf_token() }}"
                            },
