...
@section('content')
    ...
    <div class="container py-5">
        @if (request()->q || request()->kampus_id || request()->kategori_id)
            <div class="alert alert-success">Ditemukan {{ $acara->count() }} acara kegiatan</div>
        @endif
        <div class="d-flex justify-content-between align-items-center mb-2">
            <h4 class="ml-4 mb-0">Kategori Kegiatan <br>
            ...
            <form action="{{ route('landing-page.index') }}" method="GET">
                ...
                <button style="font-size: 13px;" class="mr-5 btn btn-sm btn-primary fs-14">All</button>
            </form>
        </div>
    ...
    <div class="container py-1">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <h4 class="ml-4 mb-0">Kegiatan Kampus <br>
                <small class="fs-14">Nikmatin acara kegiatan universitas</small>
            </h4>
            <form action="{{ route('landing-page.index') }}" method="GET">
                <input type="hidden" name="kampus_id" value="">
                            ...
                            <div class="col-md-6">
                                <div class="card border-0 h-100">
                                    <div class="card-img-top"
                                        style="background-size: cover; background-image: url('{{ Storage::disk('local')->url($k->thumbnail) ?: asset('img/no_image.jpeg') }}'); height: 100%; width: 100%;padding-bottom:20px;object-fit:cover;">
                                        <div class="card-body shadow-sm text-center text-white">
                                            ...
                                            <form action="{{ route('landing-page.index') }}" method="GET">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-primary fs-14 mt-3"
                                                    style="position: absolute;margin-top:-10px !important;margin-left: -60px !important;">Selengkapnya</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="container py-1" style="margin-bottom:50px;">
        {{-- Acara Terbaru --}}
                        ...
                        <div class="col-lg-3 mb-3">
                            <div class="card border-0">
                                <div class="card-body shadow-sm py-3">
                                    ...
                                    style="max-height: 150px;width:100%;object-fit:fill;"
                                        class="img-fluid">
                                    <h6 class="fs-14 fw-700 mt-3">{{ $a->judul }}</h6>
                                    <p class="fs-12 text-muted mb-0">
                                        {{ $a->waktu_mulai->translatedFormat('d M Y') }}
                                         - 
                                        {{ $a->waktu_selesai->translatedFormat('d M Y') }}
                                    </p>
                                    ...
                                    <div class="deskripsi d-flex">
                                        ...
                                        <div class="ms-4">
                                            ...
                                            <span
                                                class="fs-12">{{ $a->transaksies->where('status', 'DIBAYAR')->count() }}</span>
                                        </div>
                                        <div class="ms-4">
